...
steps{
  ...
  sh 'sudo scp -o StrictHostKeyChecking=no -i ./terraform_files/project.pem medicure-deployment.yml ubuntu@172.31.30.101:/home/ubuntu/'
  sh 'sudo scp -o StrictHostKeyChecking=no -i ./terraform_files/project.pem medicure-service.yml ubuntu@172.31.30.101:/home/ubuntu/'
script{
  try{
  sh 'ssh -o StrictHostKeyChecking=no -i ./terraform_files/project.pem ubuntu@172.31.30.101 kubectl apply -f .'
  }catch(error)
  {
  sh 'ssh -o StrictHostKeyChecking=no -i ./terraform_files/project.pem ubuntu@172.31.30.101 kubectl apply -f .'
  }
